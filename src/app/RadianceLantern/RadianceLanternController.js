...
import { RadianceLantern } from '../../config/dune';
import { getFarms } from '../../config/sflapi';
import sleep from '../../utils/sleep';

class RadianceLanternController {
  async ranking(req, res) {
    try {
      const rawdata = fs.readFileSync(
        path.resolve(CACHE_PATH, `RadianceLanternRanking.json`)
      );
  ...
  async generateFile() {
    const NUMBER_OF_PAGES = RadianceLantern.pages;
    const NUMBER_OF_FARMS = RadianceLantern.farms;
    const ua =
    ...
    try {
      await page.goto(RadianceLantern.url, {
    ...
    for (const request of farmsRequest) {
      ...
      try {
        ...
        for (const [key, value] of Object.entries(farms.data.farms)) {
          ...
          const obj = {
            FarmID: key,
            // RadianceLantern: value.inventory['Dawn Breaker Ticket'],
            // Quest: {
    ...
    // Ordena o array
    rankingResult.sort(
      (a, b) => Number(b.RadianceLantern) - Number(a.RadianceLantern)
    );
      ...
      JSON.stringify(
        {
          ...
          title: 'Radiance Lantern Ranking',
          updatedAt,
          farms: rankingResult,
        },
        null,
...
export default new RadianceLanternController();
